...
export default function AdminView(props) {
  ...
  const addNewItem = () => {
    props.addNewItem(newItemName, newItemColor, newItemPrice);
  }
  ...
  return (
    <div>
      <div>
          ...
          <div>
            Color <input type="text" onChange={ (event) => setNewItemColor(event.target.value) } />
          </div>
          <div>
            Price <input type="text" onChange={ (event) => setNewItemPrice(event.target.value) } />
          </div>
          <button onClick={ addNewItem }>Add product</button>
